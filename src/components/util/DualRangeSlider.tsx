...
export function defaultKnobBuilder(knob: 'left' | 'right', value: number) {
    return <div className='w-3 h-5 m-[2px] p-[1px] bg-slate-500 active:bg-slate-400'>
...
export default function DualRangeSlider(props: DualRangeSliderProps) {
    ...
    return <div className="flex-grow flex items-center justify-stretch gap-2">
...
function SliderHandle(props: SliderHandleProps) {
    ...
    function drag(mouseX: number, cnt: HTMLDivElement) {
        const sliderBounds = cnt.parentElement!.parentElement!.getBoundingClientRect();
        const tx = mouseX - sliderBounds.x;
        const convFactor = (props.max - props.min) / sliderBounds.width;
        const value = Math.max(props.localMin ?? props.min, Math.min(props.localMax ?? props.max, tx * convFactor + props.min));
        const knobWidth = current?.getBoundingClientRect()?.width ?? 4;
        const knobOffset = props.knob === "left" ? -knobWidth / 4 : knobWidth / 4;
    ...
    >
        {props.knob === 'right' && <div className="invisible">{props.knobBuilder(props.knob, props.value)}</div>}
        <div
            draggable={true}
            onDragStart={(e) => {
        ...
        {props.knob === 'left' && <div className="invisible">{props.knobBuilder(props.knob, props.value)}</div>}
    </div>;
}