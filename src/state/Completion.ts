...
export default class Completion {
  ...
  constructor(
    ...
    this.functions = functions
      .map((fn) => predefinedFunctions.get(fn))
      .filter((fn) => fn !== undefined) as AIFunction[];

    if (this.functions.length !== 0) {
      const extra_system_message =
        "When calling functions: If you are unsure about what parameter values to use, ask the user. However, dont be afraid to get creative, if the user wants you to. Do not write out the function calls you intend to use and dont ask for permission before executing them. Just do it! Always provide the result of a function call to the user. Write the result in a user friendly way, using markdown syntax.";
      this.system_message =
    ...
    if (message.role === "user") {
      return {
        ...
        content: message.content
          .map((cnt) => (cnt.type === "md" ? cnt.content : ""))
          .join(""),
      };
    } else {
    ...
    while (do_continue) {
      ...
      if (did_call_fn) {
        ...
        try {
          ...
          console.error(
            "API returned invalid JSON for function arguments:",
            function_args,
          );
          break;
        }
